...
int main(int argc, char *argv[]) {
    ...
    if (pid == 0) {             // 자식 프로세스인 경우
        write(fds1[1], str1, sizeof(str1));    // 파이프를 통해 첫 번째 문자열 전송
        read(fds2[0], buf, BUF_SIZE);           // 파이프로부터 데이터 읽기
        printf("Child proc output: %s \n",  buf);   // 읽은 데이터 출력
    } else {                    // 부모 프로세스인 경우
        read(fds1[0], buf, BUF_SIZE);           // 파이프로부터 데이터 읽기
        printf("Parent proc output: %s \n", buf);  // 읽은 데이터 출력
        write(fds2[1], str2, sizeof(str2));    // 파이프를 통해 두 번째 문자열 전송
        sleep(3);               // 3초 동안 대기
    }
    return 0;                   // 프로그램 종료
}
